...
MALLORIE_REGION = /(New York|NY|Pennsylvania|PA|New Jersey|NJ)/i
MALLORIE_RESPONSE = "%s belongs to Mallorie's territory!".freeze
TEILA_REGION = /(CT|MA|RI|NH|VT|ME|DE|Delaware|Connecticut|Maine|Rhode Island|New Hampshire|Vermont|Massachusetts)/i
TEILA_RESPONSE = "%s belongs to Teila's territory!".freeze
ROSS_REGION = /(OR|WA|AK|HI|Hawaii|Alaska|Washington|Oregon)/i
ROSS_RESPONSE = "%s belongs to Ross' territory!".freeze
HAYLEY_REGION = /(Socal|San Francisco|Los Angeles)/i
HAYLEY_RESPONSE = "%s belongs to Hayley's territory!".freeze
NATE_REGION = /(NV|ID|MT|WY|UT|CO|AZ|NM|KS|NE|SD|ND|MN|IA|MO|WI|IL|IN|MI|KY|OH|WV|Colorado|Nevada|Utah|Arizona|New Mexico|Wyoming|Idaho|Montana|North Dakota|South Dakota|Minnesota|Iowa|Missouri|Illinoise|Wisconsin|Indiana|Kentucky|Ohio|West Virginia|Michigan)/i
NATE_RESPONSE = "%s belongs to Nate's territory!".freeze
SEAN_REGION = /(TX|OK|AR|LA|MS|GA|FL|SC|NC|TN|VA|MD|DC|Texas|Oklahoma|Arkansas|Louisiana|Mississippi|Georgia|Flordia|South Carolina|North Carolina|Tennessee|Virginia|Maryland|Washington DC)/i
...
post '/slack/territory' do
  ...
  else "Sorry I don't recognize #{params['text']} :-("
  end
end
